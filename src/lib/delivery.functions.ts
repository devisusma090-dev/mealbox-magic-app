...
export const deliveryQueue = createServerFn({ method: "POST" })
  .inputValidator((input: { phone: string; passcode: string }) => ({
...
export const deliveryMarkOut = createServerFn({ method: "POST" })
  .inputValidator((input: { phone: string; passcode: string; id: string }) => ({
    phone: String(input.phone ?? ""),
    passcode: String(input.passcode ?? ""),
    id: String(input.id ?? ""),
  }))
...
export const deliveryCompleteByOtp = createServerFn({ method: "POST" })
  .inputValidator((input: { phone: string; passcode: string; otp: string; paymentMethod?: "cash" | "upi" }) => ({
    phone: String(input.phone ?? ""),
    passcode: String(input.passcode ?? ""),
