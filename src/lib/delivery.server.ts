...
export async function completeByOtp(db: Db, otp: string, staffPhone: string) {
  ...
  const { error } = await db
    .from("orders")
    .update({ status: "completed", completed_at: new Date().toISOString(), delivery_phone: staffPhone })
    .eq("id", order.id);
  if (error) throw new Error(error.message);
  if (order.user_id) await rewardReferrerIfFirstOrder(db as never, order.user_id);
  return { id: order.id as string, total: Number(order.total) };
}
